...
MIN_LABELS_FOR_STORY = 10     # label 数下限（排除纯跳转表）
MIN_TRANSLATABLE_FOR_STORY = 5  # 可翻译 run 数下限（有真实文本内容）


def _count_translatable(parsed_te):
    """统计 parse 后 te 的可翻译 run 数（dialog/narration/choice_*）。

    用于判定文件是否属于剧情（而不是纯跳转表）。
    """
    text = parsed_te['text']
    labels = find_labels(text)
    cnt = 0
    for li, lp in enumerate(labels):
        body_start = lp + 4
        if li + 1 < len(labels):
        ...
        for seq, (ro, rb) in enumerate(runs):
            if seq > 0:
                continue  # 只算 seq=0 的主 run
            if len(rb) < 4:
                continue
            if _is_ascii_only(rb):
                continue
            cnt += 1
    return cnt
...
# --- 正文控制码 ↔ 友好占位符 ---
#
# 游戏原文中有若干嵌入在 SJIS 文本里的控制字符，翻译时不应让翻译者手动处理。
# 提取时转成友好占位符；注入时再还原。
#
#   原文           占位符          说明
#   ---------      ---------       ------
#   "|\u3000"       "\n"            换行 + 全角空格缩进 (对话常见)
#   "|"             "\n"            换行 (无缩进)
#   "###"           (删除)          "停顿"控制码, 翻译不可见, 注入回原位
#
# 对选项块 (choice_options_*) 的 '|' 另有语义 (选项分隔), 不做转换。

# 脚本控制函数正则：$_next() / $_wait(500) / $_shake(0,0,32,4,0,40) 等
SCRIPT_FN_RE = re.compile(r'\$_\w+\([^)]*\)')


def normalize_message_for_translator(msg: str, tag: str) -> str:
    """把原始 SJIS 文本转为翻译者友好格式。
    ...
    特例：
      - '//$' 颜色码（如 `//$FFFF80`）保留原样
      - 选项块 (choice_options_*) 的 '|' 是选项分隔符，不转换

    控制码处理：
      - '$_xxx(...)' 脚本函数 (如 $_next(), $_wait(500), $_shake(...)) → **删除**
        这些是引擎内部调用，翻译者不应看到。未改动时通过 ori 自动还原。
      - '+' 半角停顿控制码 → 保留原样（翻译者自行判断）
      - '###' 停顿控制码 → 保留原样（翻译者自行判断）

    注意：好感度标记 '＋' 是 SJIS 全角 (0x81 0x7B)，和半角 '+' 不冲突。
    """
    # 选项块的 '|' 不能改
    if tag in ('choice_options_shown', 'choice_options_inner'):
        s = re.sub(r'(?<!/)/(?!/)(?!\$)', '\n', msg)
        s = SCRIPT_FN_RE.sub('', s)
        return s
...
def extract_te_file(file_path: Path, next_id: int) -> tuple:
    ...
    for li, lpos in enumerate(labels):
        ...
        for seq, (ro, rb) in enumerate(runs):
            ...
            # 原文字符串（保留所有原始控制码 |, /, 等）
            # message 是"翻译者友好"版本，所有换行符统一为 \n
            raw_msg = msg
            display_msg = normalize_message_for_translator(msg, tag)
            ...
            entry = {
                'id': cur_id,
                'message': display_msg,
                'ori': raw_msg,   # 原始字符串（保留 |/ 等），inject 时未改动场景可精确还原
                '_label': label_name,
                '_seq': seq,
                '_tag': tag,
...
def cmd_extract(input_dir: str, output_dir: str, merge: bool) -> int:
    ...
    for fp in te_files:
        ...
        translatable = _count_translatable(parsed)
        if len(labels) >= MIN_LABELS_FOR_STORY and translatable >= MIN_TRANSLATABLE_FOR_STORY:
            story_files.append(fp)
        else:
            print(f'  [skip system] {fp.name} '
                  f'(labels={len(labels)}, translatable={translatable})')
