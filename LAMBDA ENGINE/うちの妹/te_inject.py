...
def _normalize_for_compare(s: str, tag: str) -> str:
    """复制 extract 的 normalize_message_for_translator 规范化逻辑，
    用于"message 是否等于 ori 的规范化版"判定。

    规则必须与 te_extract.py::normalize_message_for_translator 严格同步！
    """
...
def denormalize_message(msg: str, tag: str) -> str:
    """把翻译者友好格式（message）转回原文控制码格式。

    - '\\n' → '|\\u3000' (标准对话换行，带全角空格缩进)
    - 选项块 (choice_options_*) 的 '|' 保留作选项分隔符
    """
    if tag in ('choice_options_shown', 'choice_options_inner'):
        # 选项块里 \n 也要还原，但选项分隔是 |，换行是 / 的情况少，一律用 / 稳妥
        return msg.replace('\n', '/')
    return msg.replace('\n', '|\u3000')


def _is_translated(entry):
    """判断条目是否被翻译（message 与 ori 不同即认为已翻译）。"""
    ori = entry.get('ori', entry.get('message', ''))
    return entry.get('message', '') != ori
...
def resolve_inject_message(entry, entries_by_id):
    ...
    if tag == 'choice_options_inner':
        ...
        if linked and _is_translated(linked):
            try:
                return apply_affection_marks(linked['message'], marks)
...
def inject_one_file(file_path, entries, out_path, verbose=False):
    ...
    for e in entries:
        ...
        # 判断 message 是否等于 normalize(ori) —— 即翻译者是否没动 message
        ori_raw = e.get('ori', e['message'])
        ori_normalized = _normalize_for_compare(ori_raw, e['_tag'])

        if new_msg == ori_normalized:
            # 未翻译：用 ori 原样（保留所有原始 |/ 控制码）
            final_msg = ori_raw
        else:
            # 已翻译：把 \n 转回 |全角空格
            final_msg = denormalize_message(new_msg, e['_tag'])

        # 编码：默认 CP932 (Shift-JIS)
        try:
            new_bytes = final_msg.encode('cp932')
        except UnicodeEncodeError as ee:
            raise ValueError(
                f"entry id={e['id']} 无法用 CP932 编码: "
                f"{ee.reason} (字符 {final_msg[ee.start:ee.end]!r})\n"
                f"  message: {final_msg!r}"
            )

        # 未改动：跳过 edit（保留原 SJIS 字节）
        if final_msg == ori_raw:
            stats['unchanged'] += 1
            continue
