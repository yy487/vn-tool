...
def find_labels(text: bytes) -> list:
    ...
    判据：字节为 '@' 且后 3 字节为 hex digits。

    注意：**不要求前字节为 \\0**。因为 Lapis 引擎存在"内嵌 label"模式
    （@XXX label 前面紧跟二进制 opcode，没有 \\0 分隔），这些内嵌 label
    同样被 code 段 `opcode 0x1E` 等引用，属于真实跳转目标。

    唯一的潜在假阳性：SJIS 双字节字符的第二字节恰好是 0x40 且紧跟 3 hex。
    但 SJIS 首字节区间 0x81-0x9F / 0xE0-0xFC 紧跟 0x40 时，0x40 是某个
    合法 SJIS 字符的第二字节（如 '　' = 0x8140），这种情况下
    我们要求前一字节不是 SJIS 首字节。
    """
    labels = []
    i = 0
    ...
    while i < n - 4:
        if (text[i] == 0x40
                and _is_hex(text[i + 1])
                and _is_hex(text[i + 2])
                and _is_hex(text[i + 3])):
            prev = text[i - 1] if i > 0 else 0
            # 排除 SJIS 双字节第二字节的假阳性
            if not ((0x81 <= prev <= 0x9F) or (0xE0 <= prev <= 0xFC)):
                labels.append(i)
                i += 4
                continue
        i += 1
