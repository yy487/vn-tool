#!/usr/bin/env python3
"""AI5WIN v4 ARC 解包/封包工具
纯索引加密解密,不含LZSS解压缩——文件原样提取/打包。

用法:
  python ai5winv4_arc_tool.py unpack <input.ARC> <output_dir>
  python ai5winv4_arc_tool.py pack   <input_dir>  <output.ARC>

ARC 格式 (从 AI5WINV4.exe FUN_004091b0 + FUN_0040a170 逆向):
  +0x00     uint32    file_count
  +0x04     Entry[N]  index_table (N × 0x108, 全部加密)
  +idx_end  ...       file_data (紧密排列, 无间隙)

Entry (0x108 = 264 bytes):
  [0x00-0xFF]   256B  filename    每字节 XOR 0x29, null填充
  [0x100-0x103]  4B   size        XOR 0x1663E1E9
  [0x104-0x107]  4B   offset      XOR 0x1BB6625C (绝对偏移)

加密常量 (EXE FUN_004091b0):
  filename_xor = 0x29
  size_xor     = 0x1663E1E9
  offset_xor   = 0x1BB6625C
"""
import struct, sys, os

# ─── 加密常量 ───
FNAME_XOR  = 0x29
XOR_SIZE   = 0x1663E1E9
XOR_OFFSET = 0x1BB6625C

ENTRY_SIZE = 0x108   # 264 bytes
FNAME_LEN  = 0x100   # 256 bytes

def dec_name(raw):
    """解密文件名 (256字节 XOR 0x29)"""
    out = bytearray(FNAME_LEN)
    for i in range(FNAME_LEN):
        out[i] = raw[i] ^ FNAME_XOR
    return out.rstrip(b'\x00').decode('ascii')

def enc_name(name):
    """加密文件名"""
    padded = name.encode('ascii').ljust(FNAME_LEN, b'\x00')[:FNAME_LEN]
    out = bytearray(FNAME_LEN)
    for i in range(FNAME_LEN):
        out[i] = padded[i] ^ FNAME_XOR
    return bytes(out)

def dec_u32(raw_val, xor_key):
    """解密 size/offset: XOR"""
    return (raw_val ^ xor_key) & 0xFFFFFFFF

def enc_u32(plain_val, xor_key):
    """加密 size/offset: XOR"""
    return (plain_val ^ xor_key) & 0xFFFFFFFF

# ─── 解包 ───
def unpack_arc(arc_path, out_dir):
    with open(arc_path, 'rb') as f:
        data = f.read()

    count = struct.unpack_from('<I', data, 0)[0]
    print(f"文件数: {count}")

    os.makedirs(out_dir, exist_ok=True)
    names = []

    for i in range(count):
        base = 4 + i * ENTRY_SIZE
        entry = data[base:base + ENTRY_SIZE]

        name   = dec_name(entry[0:FNAME_LEN])
        raw_sz = struct.unpack_from('<I', entry, 0x100)[0]
        raw_of = struct.unpack_from('<I', entry, 0x104)[0]

        size   = dec_u32(raw_sz, XOR_SIZE)
        offset = dec_u32(raw_of, XOR_OFFSET)

        fdata = data[offset:offset + size]
        out_path = os.path.join(out_dir, name)
        with open(out_path, 'wb') as fout:
            fout.write(fdata)

        names.append(name)
        print(f"  [{i+1:3d}/{count}] {name:30s}  size={size:8d}  offset=0x{offset:08X}")

    # 保存文件列表 (封包时保持顺序)
    with open(os.path.join(out_dir, '__filelist.txt'), 'w') as f:
        f.write('\n'.join(names) + '\n')

    print(f"\n解包完成: {count} 文件 → {out_dir}")

# ─── 封包 ───
def pack_arc(in_dir, arc_path):
    lst_path = os.path.join(in_dir, '__filelist.txt')
    if os.path.exists(lst_path):
        names = [l.strip() for l in open(lst_path) if l.strip()]
    else:
        names = sorted(f for f in os.listdir(in_dir)
                       if not f.startswith('__') and os.path.isfile(os.path.join(in_dir, f)))

    count = len(names)
    idx_end = 4 + count * ENTRY_SIZE   # header + index table

    # 第一遍: 收集文件数据,计算偏移
    files = []
    cur_offset = idx_end
    for name in names:
        fdata = open(os.path.join(in_dir, name), 'rb').read()
        files.append((name, fdata, cur_offset))
        cur_offset += len(fdata)

    # 构建输出
    out = bytearray()
    out += struct.pack('<I', count)

    for name, fdata, offset in files:
        enc_n = enc_name(name)
        enc_s = enc_u32(len(fdata), XOR_SIZE)
        enc_o = enc_u32(offset,     XOR_OFFSET)
        out += enc_n + struct.pack('<II', enc_s, enc_o)

    for name, fdata, offset in files:
        out += fdata

    os.makedirs(os.path.dirname(arc_path) or '.', exist_ok=True)
    with open(arc_path, 'wb') as f:
        f.write(out)

    print(f"封包完成: {count} 文件 → {arc_path} ({len(out):,} bytes)")

# ─── 入口 ───
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print(__doc__)
        sys.exit(1)

    cmd = sys.argv[1].lower()
    if cmd == 'unpack':
        unpack_arc(sys.argv[2], sys.argv[3])
    elif cmd == 'pack':
        pack_arc(sys.argv[2], sys.argv[3])
    else:
        print(f"未知命令: {cmd}")
        print(__doc__)
        sys.exit(1)
