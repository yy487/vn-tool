# -*- coding: utf-8 -*-
"""
dac_extract.py — 月下の契り / DPK引擎 .dac/.dacz 脚本文本提取器

输出 GalTransl 兼容 JSON:
  [{"id": 0, "name": "政紀", "message": "「…」"}, ...]

同时输出 .meta.json 元信息用于注入:
  {"file": "a01_01_共通01.dacz",
   "entries": [{"id": 0, "line_no": 14, "speaker_line": 12,
                "orig_name": "政紀", "orig_message": "「…」"}, ...]}

规则:
- 只提取 .call 台詞 后紧跟的单行正文 (允许中间空行)
- 跳过以全角数字+字母开头的文本行 (立绘/演出标记)
- 跳过说话人为空 "" 或 ？ 的条目里只有"？"的情况? —— 不, 旁白""也要跳 message 的 name
- name 按 ＠ 拆分, 只取 ＠ 前的显示名; 如果是空 / ？ 就设 name=None
"""
